...
export function getBridgeProc() {
  return bridgeProc;
}

export async function ensureBridgeStarted(baseUrl: string): Promise<void> {
  ...
  const cmdSetting = cfg.get<string>("command", "").trim();
  let cmd = "";
  let args: string[] = [];
...
async function guessPythonExecutable(): Promise<string> {
  ...
  try {
    ...
    if (pyExt) {
      ...
      if (interp && interp.trim()) return interp.trim();
    }
  } catch {
    // ignore
  }
