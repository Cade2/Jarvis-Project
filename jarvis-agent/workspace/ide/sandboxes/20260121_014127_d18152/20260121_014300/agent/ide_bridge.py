...
class ContextIn(BaseModel):
    active_file: Optional[str] = None
    selection: Optional[SelectionIn] = None
    buffers: Optional[Dict[str, BufferIn]] = None
...
def session_context(session_id: str, payload: ContextIn, authorization: Optional[str] = Header(default=None)) -> Dict[str, Any]:
    _require_auth(authorization)
    ...
    if payload.active_file is not None:
        s.context["active_file"] = _norm_path(payload.active_file)

    if payload.selection is not None:
        s.context["selection"] = payload.selection.model_dump()
    ...
    if payload.buffers is not None:
        ...
        for k, v in payload.buffers.items():
            nk = _norm_path(k)
            if nk and v and isinstance(v.content, str):
                buffers[nk] = v.model_dump()
